view: orquestra_cancelamento {
  ...
  dimension: motivodecancelamento {
    type: string
    sql: case when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Valor de mensalidade incorreto', 'Valor de mensalidade incorreto (15704)') then 'Valor de mensalidade incorreto' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Trancou (outros motivos)', 'Trancou (outros motivos) (15702)') then 'Trancou (outros motivos)' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Não estudou o semestre financiado', 'Não estudou o semestre financiado (15760)') then 'Não estudou o semestre financiado' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Aluno Formado', 'Aluno Formado (15692)', 'Já me formei (15692)') then 'Aluno Formado' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Bolsa / Desconto na IES', 'Bolsa / Desconto na IES (15694)') then 'Bolsa / Desconto na IES' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Curso não formou turma (15696)', 'Curso não formou turma', 'Meu curso não formou turma (15696)') then 'Curso não formou turma' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Duplicidade de contrato (15695)', 'Duplicidade de contrato') then 'Duplicidade de contrato' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Mudança de Curso (15698)', 'Mudança de Curso', 'Mudei meu curso (15698)') then 'Mudança de Curso' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Não estudei o semestre financiado (15760)', 'Não estudou o semestre financiado', 'Não estudou o semestre financiado (15760)') then 'Não estudou o semestre financiado' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Pagamento direto para a IES (15699)', 'Pagamento direto para a IES') then 'Pagamento direto para a IES' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Prouni / FIES (15693)', 'Prouni / FIES','Consegui um benefício do governo (ex Prouni/ FIES) (15694)') then 'Prouni / FIES' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Trancou (dificuldade financeira) (15700)', 'Trancou (dificuldade financeira)') then 'Trancou (dificuldade financeira)' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Transferência de IES', 'Transferência de IES (15703)') then 'Transferência de IES' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Garantidor não sabe sobre o financiamento', 'Garantidor não sabe sobre o financiamento (15697)') then 'Garantidor não sabe sobre o financiamento' when ${TABLE}."MOTIVODECANCELAMENTO" IN ('Vou pagar a mensalidade integral direto para a faculdade (15699)', 'Pagamento direto para a IES') then 'Pagamento direto para a IES' else ${TABLE}."MOTIVODECANCELAMENTO" end ;;
    group_label: "Dados do Chamado"
