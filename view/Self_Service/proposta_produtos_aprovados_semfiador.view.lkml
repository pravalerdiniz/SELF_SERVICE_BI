view: proposta_produtos_aprovados_semfiador {
    derived_table: {
      sql: WITH proposta_produtos_aprovados AS (select
        ...
        FROM "GRADUADO"."SELF_SERVICE_BI"."PROPOSTA" AS proposta
        LEFT JOIN proposta_produtos_aprovados ON (proposta."ID_PROPOSTA") = (proposta_produtos_aprovados."ID_PROPOSTA")
        LEFT JOIN "GRADUADO"."SELF_SERVICE_BI"."JORNADA" AS jornada ON (proposta."ID_PROPOSTA") = (jornada."ID_PROPOSTA")
        WHERE ((( jornada."DT_STATUS"  ) >= ((DATEADD('year', -2, DATE_TRUNC('year', CURRENT_DATE())))) AND ( jornada."DT_STATUS"  ) < ((DATEADD('year', 3, DATEADD('year', -2, DATE_TRUNC('year', CURRENT_DATE())))))))
AND (jornada."ETAPA" ) = 'Finalizado'
AND (jornada."STATUS_ETAPA" ) = 1
AND (proposta_produtos_aprovados.id_produto) in ('BOF-20','BOF-34','BOF-48','BOF-99','BOF-199','BOF-200','BOF-201','BOF-207','BOF-208')
    ...
    dimension: contagem_de_produtos {
      ...
      label: "Contagem de Produtos Sem Fiador"
      hidden: no
    }
