view: nps_nuvem_palavras_neutros {
  derived_table: {
    sql: SELECT
          T3."PALAVRAS"  AS "nps_nuvem_palavras_neutros.palavras",
            ...
            FROM
                ...
                FROM
                    ...
                    FROM
                        ...
                        WHERE (solucx."STORE_NAME") IN ('ALUNOS FORMADOS - RELACIONAL', 'ALUNOS NOVOS - RELACIONAL', 'ALUNOS RENOVAÇÃO - RELACIONAL', 'Relacional') AND (solucx."RATING_COMMENT") <> 'False' AND (solucx."CLASS_NOTA") = 'NEUTRO' AND (((solucx."TIMESTAMP") >= ((DATE_TRUNC('year', CURRENT_DATE()))) AND (solucx."TIMESTAMP") < ((DATEADD('year', 1, DATE_TRUNC('year', CURRENT_DATE())))))) AND (((( solucx."RATING_TIMESTAMP")) IS NOT NULL))
                        GROUP BY
                            1
                        ORDER BY
                            1
  ...
  dimension: nps_nuvem_palavras_neutros_palavras {
    type: string
    sql: ${TABLE}."NPS_NUVEM_PALAVRAS_NEUTROS.PALAVRAS" ;;
  }
  ...
  set: detail {
    fields: [nps_nuvem_palavras_neutros_palavras, soma_de_repeticoes]
  }
}
